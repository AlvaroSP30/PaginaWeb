document.addEventListener('DOMContentLoaded', () => {
    ...
    copyableCodeBlocks.forEach(codeBlock => {
        ...
        // Ajustar el display para que el botón se posicione correctamente
        ...
        if (codeBlock.tagName === 'CODE' && codeBlock.closest('pre')) {
            // Si es un <code> dentro de <pre>, el wrapper lo contiene
            ...
            // Si es un <code> inline sin <pre> (ej: { nombre: "Juan" })
            // Aquí no necesitamos envolverlo a menos que queramos el botón flotante.
            // Para código inline, el botón de copiar es menos común y puede ser intrusivo.
            // Si se desea, se puede ajustar el estilo del botón para que aparezca al lado, no flotante.
            // Para este caso, vamos a asumir que la mayoría de .copyable-code son bloques <pre><code>.
            // Si hay <code> sueltos, se les puede dar un estilo diferente o ignorar el botón flotante.
            // Dejaré el comportamiento para bloques y lo haré menos intrusivo para inline
            return; // No añadir botón flotante a códigos inline que no están en pre.
        }
